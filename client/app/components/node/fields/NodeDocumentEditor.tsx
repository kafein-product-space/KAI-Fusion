...
export const NodeDocumentEditor = ({ property, values, nodeType }: NodeDocumentEditorProps) => {
  ...
  const fetchFields = useCallback(async () => {
    ...
    try {
      ...
      setError(err?.message ?? "Could not load the fields");
      setFields([]);
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    const signature = JSON.stringify(dependencyValues);
          ...
          <div className="space-y-2">
            ...
            {newFieldWidget && (
              <div>
                ...
                {renderWidget(newFieldWidget as FieldInfo["widget"], newFieldValue, setNewFieldValue)}
              </div>
            )}
            ...
            <div className="flex gap-2 pt-1">
              <button
                ...
                disabled={!newFieldName.trim() || !newFieldWidget}
                onMouseDown={(e: any) => e.stopPropagation()}
                className={`px-4 py-2 rounded-lg border transition-colors text-sm ${
                  newFieldName.trim() && newFieldWidget
