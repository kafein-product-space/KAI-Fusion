...
import ReadFileToolConfigModal from "~/components/modals/tools/ReadFileToolConfigModal";

interface ReadFileToolNodeProps {
  data: any;
  id: string;
}

function ReadFileToolNode({ data, id }: ReadFileToolNodeProps) {
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2">
            <p className="font-semibold">
              {data?.displayName || data?.name || "Read File Tool"}
            </p>
          </div>
        </div>
...
export default ReadFileToolNode;
