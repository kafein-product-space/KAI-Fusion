...
import QdrantConfigModal from "~/components/modals/vector_stores/QdrantConfigModal";

interface QdrantVectorStoreNodeProps {
  data: any;
  id: string;
}

function QdrantVectorStoreNode({ data, id }: QdrantVectorStoreNodeProps) {
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2">
            <p className="font-semibold">
              {data?.displayName || data?.name || "Qdrant Vector Store"}
            </p>
          </div>
        </div>
        {/*embeddings */}
