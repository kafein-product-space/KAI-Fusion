...
class ScraplingToolNode(ProviderNode):
    ...
    def __init__(self):
        ...
        self._metadata = {
            ...
            "properties": [
                NodeProperty(
                    name="default_url",
                    displayName="Default URL (Optional)",
                    type=NodePropertyType.TEXT,
                    default="",
                    required=False,
                    placeholder="https://example.com/docs",
                    hint="Used when the agent does not pass a URL. Only public HTTP(S) URLs are accepted.",
                    tabName="basic",
                ),
                NodeProperty(
                    name="allowed_domains",
                    displayName="Allowed Domains (Optional)",
                    type=NodePropertyType.TEXT_AREA,
                    default="",
                    required=False,
                    placeholder="example.com, docs.example.com",
                    hint=(
                        "Comma/newline-separated hostname or URL allowlist. URLs are normalized to their "
                        "hostname. Exact domains and their subdomains are allowed. Leave empty to allow "
                        "any public domain."
                    ),
                    rows=3,
                    tabName="security",
                ...
                NodeProperty(
                    ...
                    default=True,
                    required=False,
                    hint="Checks robots.txt before every page request. Recommended for compliant crawling.",
                    tabName="security",
                ),
                NodeProperty(
                    ...
                    hint="What to do when robots.txt cannot be checked because of a network/server error.",
                    tabName="security",
                ),
                NodeProperty(
                    name="verify_ssl",
                    ...
                    default=True,
                    required=False,
                    hint="Keep enabled in production.",
                    tabName="security",
                ),
                NodeProperty(
                    name="tls_profile",
                    displayName="HTTP TLS Profile (No Browser)",
                    type=NodePropertyType.SELECT,
                    default="chrome",
                    required=False,
                    options=[
                        {"label": "Chrome HTTP profile", "value": "chrome"},
                        {"label": "Firefox HTTP profile", "value": "firefox"},
                        {"label": "Safari HTTP profile", "value": "safari"},
                        {"label": "Edge HTTP profile", "value": "edge"},
                        {"label": "No impersonation", "value": "none"},
                    ],
                    hint="curl_cffi TLS fingerprint only; it does not start a browser.",
                    tabName="request",
                ),
                NodeProperty(
                ...
                NodeProperty(
                    ...
                    required=False,
                    hint="Hard cap; the agent cannot override it.",
                    tabName="limits",
                ),
                NodeProperty(
                    name="crawl_delay_ms",
                ...
                NodeProperty(
                    ...
                    max=200000,
                    required=False,
                    hint="Bounds content sent back to the agent context.",
                    tabName="limits",
                ),
            ],
            "version": "1.0.0",
    ...
    def get_required_packages(self) -> List[str]:
        """Return the supported Scrapling package set used by this node."""
        return [
            "scrapling==0.4.14",
            "curl-cffi==0.16.0",
            "browserforge==1.2.4",
