...
class Database:
    ...
    def _ensure_client(self) -> Client:
        ...
        if not self.client:
            raise ValueError(
                "Database client not available. Check Supabase configuration."
            )
        return self.client
    ...
    async def get_workflow(
        ...
        try:
            ...
            # Permission rules:
            # • If caller is owner → always allowed.
            # • If workflow is public → allowed for anyone.
            # • Otherwise → denied.

            if user_id:
                if workflow["user_id"] != user_id and not workflow.get(
                    "is_public", False
                ):
                    logger.warning(
                        f"Access denied to workflow {workflow_id} for user {user_id}"
                    )
                    return None
            else:
                # Unauthenticated caller: only allow public workflows
                if not workflow.get("is_public", False):
                    logger.warning(
                        f"Unauthenticated access denied for private workflow {workflow_id}"
                    )
                    return None
    ...
    async def update_workflow(
        self, workflow_id: str, user_id: str, data: dict
    ) -> Optional[dict]:
        """Update a workflow"""
        client = self._ensure_client()
        try:
    ...
    async def create_execution(
        self, workflow_id: str, user_id: str, inputs: dict
    ) -> dict:
    ...
    async def update_execution(
        ...
        try:
            ...
            client.table("executions").update(update_data).eq(
                "id", execution_id
            ).execute()
    ...
    async def get_workflow_executions(
        self, workflow_id: str, user_id: str, limit: int = 10, offset: int = 0
    ) -> List[dict]:
    ...
    async def _delete_workflow_executions(self, workflow_id: str, user_id: str):
        ...
        try:
            client = self._ensure_client()
            client.table("executions").delete().eq("workflow_id", workflow_id).eq(
                "user_id", user_id
            ).execute()
        except Exception as e:
            logger.warning(
                f"Failed to delete executions for workflow {workflow_id}: {e}"
            )
    ...
    async def create_credential(self, credential_data: dict) -> dict:
        ...
        client = self._ensure_client()
        try:
            result = client.table("credentials").insert(credential_data).execute()

            if result.data:
                logger.info(f"✅ Created credential: {credential_data.get('name')}")
                return result.data[0]
            else:
                raise ValueError("No data returned from credential creation")

        except Exception as e:
            logger.error(f"❌ Failed to create credential: {e}")
            raise

    async def get_credential(self, credential_id: str, user_id: str) -> Optional[dict]:
    ...
    async def get_user_credentials(
        self, user_id: str, service_type: Optional[str] = None
    ) -> List[dict]:
    ...
    async def update_credential(
        self, credential_id: str, update_data: dict
    ) -> Optional[dict]:
        """Update a credential"""
        client = self._ensure_client()
        try:
    ...
    async def get_custom_nodes(self, user_id: Optional[str] = None) -> List[dict]:
        """Get custom nodes (public and user's own)"""
        client = self._ensure_client()
        try:
            query = client.table("custom_nodes").select("*")
    ...
    async def get_stats(self, user_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if user_id:
                ...
                nodes_result = (
                    self.client.table("custom_nodes")
                    .select("id")
                    .eq("user_id", user_id)
                    .execute()
                )
            ...
            if not user_id:
                ...
                total_executions = (
                    self.client.table("executions").select("id").execute()
                )
    ...
    async def create_task(
        self, user_id: str, celery_task_id: str, task_type: str, data: dict
    ) -> dict:
        ...
        try:
            ...
            result = client.table("async_tasks").insert(task_data).execute()
    ...
    async def list_user_tasks(
        ...
        client = self._ensure_client()
        try:
            query = client.table("async_tasks").select("*").eq("user_id", user_id)
            ...
            count_result = (
                client.table("async_tasks").select("id").eq("user_id", user_id)
            )
            if status:
    ...
    async def get_task_statistics(
        ...
        client = self._ensure_client()
        try:
            query = client.table("task_statistics").select("*").eq("user_id", user_id)
            ...
            if result.data:
                ...
                stats = {
                    ...
                    "completed_tasks": sum(
                        row["completed_tasks"] for row in result.data
                    ),
                ...
                if total_completed > 0:
                    weighted_exec_time = (
                        sum(
                            (row["avg_execution_time"] or 0)
                            * (row["completed_tasks"] + row["failed_tasks"])
                            for row in result.data
                            if row["avg_execution_time"]
                        )
                        / total_completed
                        if any(row["avg_execution_time"] for row in result.data)
                        else None
                    )
    ...
    async def add_task_log(
        ...
        try:
            ...
            if step_number is not None:
                log_data["step_number"] = str(step_number)

            client.table("task_execution_logs").insert(log_data).execute()
            return True
    ...
    async def get_task_logs(
        self, task_id: str, user_id: str, limit: int = 100
    ) -> List[dict]:
        ...
        try:
            ...
            result = (
                client.table("task_execution_logs")
                .select("*")
                .eq("task_id", task_id)
    ...
    async def cleanup_old_tasks(self) -> int:
        ...
        try:
            ...
            result = client.rpc("cleanup_old_tasks").execute()
    ...
    # ------------------------------
    # Public workflows (no auth)
    # ------------------------------

    async def get_public_workflows(self) -> List[dict]:
        """Return all public workflows (is_public = true)."""
        client = self._ensure_client()
        try:
            result = (
                client.table("workflows")
            ...
            logger.error(f"❌ Failed to get public workflows: {e}")
            return []
...
def get_database() -> Database:
    ...
    if db is None:
        raise HTTPException(
            status_code=503,
            detail="Database not available. Check SUPABASE_URL and SUPABASE_KEY configuration.",
        )
    return db
