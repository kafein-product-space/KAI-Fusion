"""Small canonical catalog for Model Security findings.

The scanners keep their native identifiers (ModelAudit ``Sxxx`` rules and
Fickling analysis names) inside their service boundary.  Workflow output uses
the stable, product-owned code assigned here instead.  ModelAudit's English
catalog is loaded from the pinned dependency at runtime; Fickling has no
equivalent catalog, so its small set of analysis/error definitions lives here.
"""
...
MODELAUDIT_SOURCE_URL = (
    "https://github.com/promptfoo/modelaudit/blob/v0.2.52/"
    "modelaudit/rule_catalog.py"
)
FICKLING_ANALYSIS_SOURCE_URL = (
    "https://github.com/trailofbits/fickling/blob/v0.1.12/"
    "fickling/analysis.py"
)
FICKLING_LOADER_SOURCE_URL = (
    "https://github.com/trailofbits/fickling/blob/v0.1.12/"
    "fickling/loader.py"
)
...
def _risk_level(value: Any) -> str:
    """Normalize ModelAudit/Fickling severity names and numeric values."""
...
def _modelaudit_translation(code: str, fallback: str) -> str:
    return MODELAUDIT_TURKISH.get(
        code,
...
def _load_modelaudit_entries() -> list[SecurityErrorDefinition]:
    try:
    ...
    for native in RULE_CATALOG:
        ...
        guidance_en, guidance_tr = _guidance("ModelAudit", code)
        entries.append(
            SecurityErrorDefinition(
                ...
                turkish_description=_modelaudit_translation(code, description),
                guidance_en=guidance_en,
                guidance_tr=guidance_tr,
                source_url=MODELAUDIT_SOURCE_URL,
            )
        )
    return entries
...
def _fickling_entry(
    ...
    *,
    source_condition: str = "",
    source_url: str = FICKLING_ANALYSIS_SOURCE_URL,
) -> SecurityErrorDefinition:
    guidance_en, guidance_tr = _guidance("Fickling", source_code)
...
def _load_fickling_entries() -> list[SecurityErrorDefinition]:
    return [
        _fickling_entry(
        ...
        _fickling_entry(
            ...
            source_condition="Fickling severity is OVERTLY_MALICIOUS.",
        ),
        _fickling_entry(
            "OvertlyBadEval",
            "high",
            ...
            source_condition="Fickling severity is LIKELY_UNSAFE.",
        ),
        _fickling_entry(
            "UnsafeImports",
            "critical",
        ...
        _fickling_entry(
            ...
            source_condition="Fickling severity is SUSPICIOUS.",
        ),
        _fickling_entry(
            "MLAllowlist",
            "high",
        ...
        _fickling_entry(
            ...
            "medium",
            "File read error",
            "Fickling could not read the target file, so the security result is incomplete.",
            "Fickling hedef dosyayı okuyamadı; bu nedenle güvenlik sonucu eksik kaldı.",
            source_condition="Fickling loader error prefix: File error.",
            source_url=FICKLING_LOADER_SOURCE_URL,
        ),
        _fickling_entry(
            "ReadError",
            "medium",
            "Archive member read error",
            "Fickling could not read an archive member, so the security result is incomplete.",
            "Fickling bir arşiv üyesini okuyamadı; bu nedenle güvenlik sonucu eksik kaldı.",
            source_condition="Fickling loader error prefix: Read error.",
            source_url=FICKLING_LOADER_SOURCE_URL,
        ),
        _fickling_entry(
            "ArchiveError",
            "medium",
            "Archive read error",
            "Fickling could not open the archive, so the security result is incomplete.",
            "Fickling arşivi açamadı; bu nedenle güvenlik sonucu eksik kaldı.",
            source_condition="Fickling loader error prefix: Archive error.",
            source_url=FICKLING_LOADER_SOURCE_URL,
        ),
        _fickling_entry(
            "AnalysisError",
            ...
            "Fickling failed while running a security analysis, so the result must be treated as inconclusive.",
            "Fickling güvenlik analizini çalıştırırken başarısız oldu; sonuç kesin olmayan kabul edilmelidir.",
            source_condition="Fickling loader error prefix: Analysis error.",
            source_url=FICKLING_LOADER_SOURCE_URL,
        ),
        _fickling_entry(
            "ParseError",
            "high",
            "Pickle parse error",
            "Fickling could not parse the pickle; malformed or intentionally evasive content may be present.",
            "Fickling pickle dosyasını ayrıştıramadı; bozuk veya kasıtlı olarak kaçamak hazırlanmış içerik olabilir.",
            source_condition="Fickling loader error prefix: Parse error.",
            source_url=FICKLING_LOADER_SOURCE_URL,
        ),
        _fickling_entry(
            "UnexpectedError",
            ...
            "Fickling raised an unexpected error, so the security result is not conclusive.",
            "Fickling beklenmeyen bir hata verdi; bu nedenle güvenlik sonucu kesin değildir.",
            source_condition="Fickling loader error prefix: Unexpected error.",
            source_url=FICKLING_LOADER_SOURCE_URL,
        ),
    ]
...
def get_security_error_catalog() -> tuple[SecurityErrorDefinition, ...]:
    ...
    entries = [*_load_modelaudit_entries(), *_load_fickling_entries()]
...
def _canonical_engine(value: Any) -> str:
    ...
    if token in {"modelaudit", "staticanalysis"}:
        return "ModelAudit"
    if token in {"fickling", "picklesecurity", "pickleanalysis"}:
        return "Fickling"
    return str(value or "").strip()


def infer_fickling_source_code(value: Any) -> str:
    """Infer a stable Fickling source key from a result or loader error string."""
...
def find_security_error(
    ...
    *,
    ...
    if engine_name == "Fickling":
        source = infer_fickling_source_code(source) or source
...
def enrich_security_finding(
    ...
    *,
    ...
    source = str(source_code or "").strip()
    if _canonical_engine(engine) == "Fickling":
        source = infer_fickling_source_code(source) or source
...
# Turkish translations for the pinned ModelAudit catalog.  Keeping these beside
